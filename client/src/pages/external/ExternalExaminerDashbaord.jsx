...
const ExternalExaminerDashboard = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/projects/external-pending');
  ...
  const handleSubmitGrade = async () => {
    ...
    try {
      setProcessing(true);
      const response = await fetch(`http://localhost:5000/api/projects/submit-final-marks/${selectedProject._id}`, {
        method: 'PUT',
        headers: {
